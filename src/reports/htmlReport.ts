...
import { APP_NAME, AUTHOR_CREDIT, AUTHOR_EMAIL } from '@/lib/branding';
import {
  formatDateTime,
  formatDuration,
  formatFrequency,
...
<title>${escapeHtml(APP_NAME)} report - ${escapeHtml(session.name)}</title>
<style>${CSS}</style>
</head>
<body>
...
<p class="sub">${escapeHtml(APP_NAME)} &mdash; calibrated smartphone acoustic measurement tool. Not an IEC 61672 classified sound level meter.</p>
...
<div class="foot">
Generated by ${escapeHtml(APP_NAME)} on ${escapeHtml(formatDateTime(Date.now()))}.
...
${escapeHtml(AUTHOR_CREDIT)} &middot; ${escapeHtml(AUTHOR_EMAIL)}.
</div>
</body>
</html>`;
}
